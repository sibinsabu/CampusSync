...
Future<String?> _saveImageToLocal(Uint8List imageBytes, String eventId) async {
  try {
    ...
    final eventsDir = Directory(path.join(directory.path, 'event_covers'));
    ...
    final filePath = path.join(eventsDir.path, fileName);
...
Future<bool> _localImageExists(String filePath) async {
  try {
    final file = File(filePath);
    return await file.exists();
  } catch (e) {
    debugPrint('Error checking local image: $e');
    return false;
  }
}

// Helper function to load image from local storage
Future<File?> _getLocalImage(String filePath) async {
  try {
    final file = File(filePath);
...
bool _isValidImageUrl(String? url) {
  ...
  try {
    ...
    debugPrint(
      'URL validation: scheme=${uri.scheme}, hasScheme=${uri.hasScheme}, isValid=$isValid',
    );
    return isValid;
  } catch (e) {
...
class _AddEventPageState extends State<AddEventPage> {
  ...
  Future<void> _publishEvent() async {
    ...
    try {
      ...
      debugPrint('=== EVENT CREATION DEBUG ===');
      debugPrint('Cover image bytes null: ${_coverImageBytes == null}');
      debugPrint('Cover image bytes length: ${_coverImageBytes?.length}');

      if (_coverImageBytes != null) {
        debugPrint('Starting local image save...');

        // Generate a temporary event ID for filename
        ...
        try {
          ...
          if (localImagePath != null) {
            debugPrint('✅ Image saved locally: $localImagePath');
          } else {
            debugPrint('❌ Failed to save image locally');
          }
        } catch (saveError) {
          debugPrint('❌ Error saving image locally: $saveError');
          // Continue without image if save fails
          localImagePath = null;
        }
      } else {
        debugPrint('⚠️ No cover image provided - creating event without image');
      }

      final parsedDate = _selectedDate;
      ...
      // Create event in Firestore first to get the event ID
      final eventDoc = await FirebaseFirestore.instance
          .collection('events')
          .add({
            'title': title,
      ...
      debugPrint(
        '✅ Event saved to Firestore with localImagePath: $localImagePath',
      );
      debugPrint('✅ Event ID: ${eventDoc.id}');
            ...
            builder: (context, snapshot) {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, i) {
                  ...
                  debugPrint('=== LOCAL IMAGE DEBUG ===');
                  debugPrint('Event: $title');
                  debugPrint('Raw localImagePath: ${data['localImagePath']}');
                  debugPrint(
                    'LocalImagePath type: ${data['localImagePath'].runtimeType}',
                  );
                  debugPrint(
                    'LocalImagePath null check: ${data['localImagePath'] == null}',
                  );
                  debugPrint(
                    'LocalImagePath empty check: ${data['localImagePath'].toString().isEmpty}',
                  );
                  debugPrint('==================');
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (localImagePath != null &&
                            ...
                            FutureBuilder<File?>(
                              future: _getLocalImage(localImagePath),
                              builder: (context, snapshot) {
                                ...
                                return Container(
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text(
                                        'Path: ${localImagePath.length > 30 ? localImagePath.substring(0, 30) + "..." : localImagePath}',
                                        style: const TextStyle(
                                          color: Colors.red,
                                          fontSize: 8,
                                        ),
...
class _EventDetailsPageState extends State<EventDetailsPage> {
  ...
  Future<void> _registerForEvent() async {
    ...
    if (regDocs.docs.any((doc) => doc.id == user.uid)) {
      if (mounted)
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
    ...
    final isPaidEvent = widget.eventData['isPaid'] == true;
    final price = widget.eventData['price'] ?? 0.0;

    if (isPaidEvent && price > 0) {
    ...
    try {
      await FirebaseFirestore.instance
          ...
          .set({
            ...
            'paymentAmount': widget.eventData['isPaid'] == true
        ...
        Navigator.of(context).pushReplacement(
          MaterialPageRoute(
            builder: (context) => RegistrationSuccessPage(
              eventId: widget.eventId,
              eventData: widget.eventData,
              isPaidEvent: widget.eventData['isPaid'] == true,
            ),
          ),
        );
      }
      ...
      builder: (context) => Dialog(
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              Text(
                widget.eventData['title'] ?? 'Event QR Code',
                style: const TextStyle(
                  fontSize: 20,
                  fontWeight: FontWeight.bold,
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(
                      '${user.uid.substring(0, 8).toUpperCase()}-${widget.eventId.substring(0, 8).toUpperCase()}',
                      style: const TextStyle(
                        fontSize: 16,
                        fontWeight: FontWeight.bold,
              ...
              const SizedBox(height: 8),
              Text(
                'Show this code at the event venue',
        ...
        slivers: [
          SliverAppBar(
            ...
            flexibleSpace: FlexibleSpaceBar(
              background: localImagePath != null && localImagePath.isNotEmpty
                  ? FutureBuilder<File?>(
                      future: _getLocalImage(localImagePath),
                      builder: (context, snapshot) {
                        if (snapshot.connectionState ==
                  ...
                  : _isValidImageUrl(data['imageUrl'] as String?)
                  ? CachedNetworkImage(
                      ...
                      errorWidget: (context, url, error) {
                        debugPrint('EventDetails image load error: $error');
                        return Container(
                          color: const Color(0xFF3F51B5),
                          child: const Column(
                            mainAxisAlignment: MainAxisAlignment.center,
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildDetailRow(
                          ...
                          'Time',
                          data['eventTime'] ?? 'No time specified',
                          Colors.green,
                        ),
                      ],
                    ),
                  ...
                  StreamBuilder<DocumentSnapshot>(
                    stream: FirebaseAuth.instance.currentUser != null
                        ? FirebaseFirestore.instance
                              ...
                              .snapshots()
                        : Stream.empty(),
                    builder: (context, userRegSnapshot) {
                      final isRegistered =
                          userRegSnapshot.data?.exists ?? false;

                      if (isRegistered) {
                        // User is registered - show QR and Unregister buttons
                        return Column(
                          children: [
                            Container(
                              width: double.infinity,
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 8),
                    if (widget.eventData['venue'] != null)
                      Row(
                        children: [
                          Icon(
                            Icons.location_on_outlined,
                    ...
                    if (widget.eventData['eventDate'] != null)
                      Row(
                        children: [
                          ...
                          Text(
                            DateTime.fromMillisecondsSinceEpoch(
                              widget.eventData['eventDate'] as int,
                            ).toLocal().toString().split(' ')[0],
                            style: TextStyle(
                              fontSize: 14,
                              color: Colors.grey[700],
          ...
          child: Row(
            children: [
              ...
              Radio<String>(
                ...
                fillColor: WidgetStateProperty.all(const Color(0xFF3F51B5)),
              ),
            ],
          ),
        ),
...
class _RegistrationSuccessPageState extends State<RegistrationSuccessPage>
  ...
  void initState() {
    ...
    // Complete registration after page loads
    ...
    try {
      await FirebaseFirestore.instance
          ...
          .set({
            ...
            'registeredAt': FieldValue.serverTimestamp(),
            'paymentStatus': 'completed',
            'paymentAmount': widget.paymentAmount ?? 0.0,
            'paymentMethod': widget.paymentMethod ?? 'free',
          });
    } catch (e) {
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  'You have successfully registered for ${widget.eventData['title'] ?? 'the event'}',
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (widget.isPaidEvent && widget.paymentAmount != null)
                        _buildInfoRowWithIcon(
                        ...
                        onPressed: () {
                          Navigator.of(context)
                              ...
                              .then((_) {
                                ...
                                final mainNavigationHolder = context
                                    .findAncestorStateOfType<
                                      _MainNavigationHolderState
                                    >();
                                mainNavigationHolder?.setState(() {
                                  mainNavigationHolder._selectedIndex = 1;
                                });
                              });
                        },
                        style: ElevatedButton.styleFrom(
...
class _YourEventsPageState extends State<YourEventsPage> {
  String _searchQuery = '';
  String _filterStatus = 'all'; // all, upcoming, past, free, paid
  ...
  List<DocumentSnapshot> _filterEvents(List<DocumentSnapshot> events) {
    if (_searchQuery.isEmpty && _filterStatus == 'all') return events;
    ...
    return events.where((event) {
      ...
      if (_searchQuery.isNotEmpty) {
        ...
        final venue = (eventData['venue'] as String? ?? '').toLowerCase();
      ...
      switch (_filterStatus) {
        case 'upcoming':
          final eventDate = eventData['eventDate'] as int?;
    ...
    return Padding(
      ...
      child: FilterChip(
        ...
        labelStyle: TextStyle(
          color: isSelected ? const Color(0xFF3F51B5) : Colors.grey.shade700,
  ...
  Widget _buildEventsList() {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        // We need to get event details for each registration
        return FutureBuilder<List<Map<String, dynamic>>>(
          future: _getEventDetails(registrations),
          builder: (context, futureSnapshot) {
            ...
            if (futureSnapshot.hasError) {
              return Center(
                child: Text(
                  'Error loading event details: ${futureSnapshot.error}',
                  style: TextStyle(color: Colors.red[400]),
                  textAlign: TextAlign.center,
                ),
              );
            ...
            final eventsWithDetails = futureSnapshot.data ?? [];
            final filteredEvents = _filterEvents(
              registrations.where((reg) {
                final eventId = reg.reference.parent.parent?.id ?? '';
                return eventsWithDetails.any(
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                final regDoc = filteredEvents[index];
                final eventId = regDoc.reference.parent.parent?.id ?? '';
                final eventData =
                    eventsWithDetails.firstWhere(
                          (event) => event['eventId'] == eventId,
                          orElse: () => {'eventData': {}},
                        )['eventData']
                ...
                if (eventData == null) {
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      title: const Text('Event details unavailable'),
                      subtitle: const Text('This event may have been deleted'),
                    ),
                  );
                }
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          SizedBox(
                            ...
                            child: ElevatedButton.icon(
                              onPressed: () =>
                                  _showQRCode(context, eventData, eventId),
                              icon: const Icon(Icons.qr_code, size: 18),
                              label: const Text('Show QR Code'),
                              style: ElevatedButton.styleFrom(
                                backgroundColor: const Color(0xFF3F51B5),
  ...
  Future<List<Map<String, dynamic>>> _getEventDetails(
    List<DocumentSnapshot> registrations,
  ) async {
    final List<Map<String, dynamic>> eventsWithDetails = [];
